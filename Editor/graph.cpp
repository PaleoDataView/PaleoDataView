    ...
    : diag(dialog)
{
    ...
    for (size_t i=0;i<number;i++){
        ...
        repeat_x[i]=0;
        repeat_x1[i]=-3.40282e+10;
        repeat_x2[i]=3.40282e+10;
        repeat_y[i]=0;
        repeat_y1[i]=-3.40282e+10;
        repeat_y2[i]=3.40282e+10;

        limit_x1[i]=-3.40282e+10;
        limit_x2[i]=3.40282e+10;
        limit_y1[i]=-3.40282e+10;
        limit_y2[i]=3.40282e+10;
...
void Graph::setTicks(int tx,int ty){
    setTicks(tx,ty,0);
}

void Graph::setTicks(int tx,int ty,int n){
    ticks_x[n]=tx;
    ticks_y[n]=ty;
...
void Graph::setColorLegend(int type,QString t,float min, float max){
    setColorLegend(type,t,min,max,0);
}

void Graph::setColorLegend(int type,QString t, float min, float max, int i){
    ...
    colorLegend_titel[i]->clear();
    colorLegend_titel[i]->append(t);
    colorLegend_min[i]=min;
    colorLegend_max[i]=max;
    gobject->setColorLegend(&colorLegend_type[0]);
}

void Graph::setColorScheme(int i){
    setColorScheme(i,0);
}

void Graph::setColorScheme(int i,int n){
    colorScheme[n]=i;
...
int Graph::getColorScheme(int n){
...
void Graph::mouseReleaseEvent(QGraphicsSceneMouseEvent* mouseEvent2){
    ...
    if(moveFlag==0 && key_flag==0){
        ...
        if (repeat_x[focus]>0||repeat_y[focus]>0){
        ...
        qDebug()<<QString::number(new_x_min)+":"+QString::number(new_x_max)+":"+QString::number(new_y_min)+":"+QString::number(new_y_max);
        qDebug()<<"found:"+QString::number(select_x.size());
        ...
        if (select_x.length()>0){
            ...
            qDebug()<<"x:"+QString::number(select_x.at(select_x.length()-1))+" Y:"+QString::number(select_y.at(select_x.length()-1));
            QString str="";
            ...
            QToolTip::hideText();
            QToolTip::showText(pos,str,nullptr);
...
void Graph::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *mouseEvent5){
    ...
    qDebug()<<"Double"+QString::number(new_x_min)+" - "+QString::number(new_x_max)+" : "+QString::number(new_y_min)+" - "+QString::number(new_y_max);
    // create QList of all selected points
    select_plot=focus;
    select_x.clear();
    select_y.clear();
...
void Graph::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    if (colorScheme[focus]>=0 ) {
        QMenu* colorMenu;
        colorMenu = menu.addMenu(tr("Color Scheme"));
            ...
            a1->setText("Spectrum");
            colorMenu->addAction(a1);

            QAction* a2 = new QAction();
                                    a2->setCheckable(1);
                                    if (colorScheme[focus]==1) {
                                        a2->setChecked(1);
                                    }
                                    a2->setText("Spectrum(reverse)");
                                    colorMenu->addAction(a2);

                        QAction* a3 = new QAction();
                                    a3->setCheckable(1);
                                    if (colorScheme[focus]==2) {
                                        a3->setChecked(1);
                                    }
                                    a3->setText("Greyscale");
                                    colorMenu->addAction(a3);
                        QAction* a4 = new QAction();
                                    a4->setCheckable(1);
                                    if (colorScheme[focus]==3) {
                                        a4->setChecked(1);
                                    }
                                    a4->setText("Redscale");
                                    colorMenu->addAction(a4);
                        QAction* a5 = new QAction();
                                    a5->setCheckable(1);
                                    if (colorScheme[focus]==4) {
                                        a5->setChecked(1);
                                    }
                                    a5->setText("Greenscale");
                                    colorMenu->addAction(a5);
                        QAction* a6 = new QAction();
                                    a6->setCheckable(1);
                                    if (colorScheme[focus]==5) {
                                        a6->setChecked(1);
                                    }
                                    a6->setText("Bluescale");
                                    colorMenu->addAction(a6);
    ...
    if (a)
    {
        ...
        if (a->iconText()=="Graph Options"){
            delete editgraph;

            editgraph=new EditGraph(diag);
            connect(editgraph,SIGNAL(graphChangedEdit()),this,SLOT(transmitGraphChanged()));
            // Transfer all information to editor -> by reference -> changes made affect original data
            editgraph->setNumber(number);
            for (int i=0;i<number;i++) {
                editgraph->setData(data[i],column[i],row[i],i);
                editgraph->setUse(useflag[i],usemode[i],i);
                editgraph->setError(data_err[i],error[i],i);
                editgraph->setError2(data_err2[i],error2[i],i);

                editgraph->setTitel(*titel[i],*titel_x[i],*titel_y[i],i);
                editgraph->setTextSize(textsize[i],titelsize[i],margin[i],i);

                editgraph->setSymbol(symbol_size[i],i);
                editgraph->setColor(col_dat[i],color[i],i);

                editgraph->setComment(dat_com[i],com_mode[i],i);
                editgraph->setComHide(com_hide[i], i);
                editgraph->setLabel(dat_label[i],label_mode[i],i);

                editgraph->setLineWidth(line_width[i],i);
                editgraph->setLineStyle(line_style[i],i);
                editgraph->setLineColor(line_color[i],i);

                editgraph->setMultiplicator(factor_x[i],factor_y[i],i);

                editgraph->setMark(data_mark[i],marker[i],i);
                editgraph->addMarker(marker_x[i],marker_y[i],marker_mode[i],i);

                editgraph->fixRange(fix_x[i],fix_y[i],i);
                editgraph->setLockAspect(lock_aspect[i],i);
                editgraph->setFolding(fold_x[i],fold_y[i],i);
                editgraph->setRepeat(repeat_x[i],repeat_x1[i],repeat_x2[i],repeat_y[i],repeat_y1[i],repeat_y2[i],i);
                editgraph->setLimit(limit_x1[i],limit_x2[i],limit_y1[i],limit_y2[i],i);

                editgraph->setAxisType(x_axis_type[i],y_axis_type[i],i);
                editgraph->setTicks(ticks_x[i],ticks_y[i],i);
                if (*background_filename[i]!="" && background[i]>0) editgraph->setBackground(*background_filename[i],background_x1[i],background_x2[i],background_y1[i],background_y2[i],background[i],i);
                if (background_fileimage[i]!=nullptr && background[i]>0) editgraph->setBackground(background_fileimage[i],background_x1[i],background_x2[i],background_y1[i],background_y2[i],background[i],i);
                if (background_image[i]!=nullptr && background[i]>0) editgraph->setBackground2(background_image[i],background_x1[i],background_x2[i],background_y1[i],background_y2[i],background[i],i);

                editgraph->setColorLegend(colorLegend_type[i],*colorLegend_titel[i],colorLegend_min[i],colorLegend_max[i],i);

                editgraph->setColorScheme(colorScheme[i],i);

                editgraph->setSetPoly(setpoly[i],setpoly_mode[i],i);
                editgraph->setSetArea(setarea_x[i],setarea_y[i],setarea_mode[i],i);

                editgraph->setSetColor(setcolor_color[i],setcolor_mode[i],i);
                editgraph->setSetLineColor(setlinecolor_color[i],setlinecolor_mode[i],i);
                editgraph->setSetLineWidth(setlinewidth[i],setlinewidth_mode[i],i);
                editgraph->setSetSymbolsize(setsymbolsize[i],setsymbolsize_mode[i],i);
                editgraph->setSetLineStyle(setlinestyle[i],setlinestyle_mode[i],i);
                editgraph->setSetSymboltype(setsymboltype[i],setsymboltype_mode[i],i);



                editgraph->setAutosize(autosize[i],autosize_mode[i],i);
                editgraph->setSettings(path[i]->toLatin1(),i);
                editgraph->autoSize(i);

            }
            // transfer object data
            int count=gobject->getCount();
            editgraph->addLineClear();
            for (int i=0;i<count;i++){
                editgraph->addLine(gobject->getLine_x1(i),gobject->getLine_y1(i),gobject->getLine_n1(i),
                                   gobject->getLine_x2(i),gobject->getLine_y2(i),gobject->getLine_n2(i),
                                   gobject->getLine_Color(i),gobject->getLine_Width(i),gobject->getLine_Style(i));
            }
            editgraph->setAddMode(gobject->getAddMode());


            editgraph->updateAllTabs();
            editgraph->setModal(true);
            editgraph->setWindowFlags(Qt::Window | Qt::WindowTitleHint | Qt::WindowMaximizeButtonHint | Qt::WindowCloseButtonHint | Qt::CustomizeWindowHint);
            editgraph->show();
        ...
        if (a->iconText()=="Spectrum"){
                            setColorScheme(0,focus);
                            emit(graphChanged());
                        }
            if (a->iconText()=="Spectrum(reverse)"){
                            setColorScheme(1,focus);
                            emit(graphChanged());
                        }
                if (a->iconText()=="Greyscale"){
                            setColorScheme(2,focus);
                            emit(graphChanged());
                        }
                if (a->iconText()=="Redscale"){
                            setColorScheme(3,focus);
                            emit(graphChanged());
                        }
                if (a->iconText()=="Greenscale"){
                            setColorScheme(4,focus);
                            emit(graphChanged());
                        }
                if (a->iconText()=="Bluescale"){
                            setColorScheme(5,focus);
                            emit(graphChanged());
                        }
...
void Graph::setLimit(float x1, float x2, float y1, float y2, int n){
    limit_x1[n]=x1;
    limit_x2[n]=x2;
    limit_y1[n]=y1;
    limit_y2[n]=y2;
...
void Graph::setLimit(float x1, float x2, float y1, float y2){
...
void Graph::setView(float x1,float x2,float y1,float y2,int n){
    x_max[n]=x2;
    x_min[n]=x1;
    y_min[n]=y1;
    y_max[n]=y2;
...
void Graph::setView(float x1,float x2,float y1,float y2){
    setView(x1,x2,y1,y2,0);
}

float Graph::get_Min_X(int i){
    x_min[i]=gdata[i]->get_x_min();
    return x_min[i];
}
float Graph::get_Min_Y(int i){
    y_min[i]=gdata[i]->get_y_min();
    return y_min[i];
}
float Graph::get_Max_X(int i){
    x_max[i]=gdata[i]->get_x_max();
    return x_max[i];
}
float Graph::get_Max_Y(int i){
    y_max[i]=gdata[i]->get_y_max();
    return y_max[i];
}

void Graph::transmitGraphChanged(){
   emit(graphChanged());
}
